...
public class VRSL_AudioDMX : UdonSharpBehaviour
{
    public UnityEngine.UI.Text[] dmxBoolTexts, dmxValueTexts;
    public UnityEngine.UI.Text rawBandValueText, upperTargetMultiplierSliderText, channel7SliderText, channel8SliderText;
  //[HideInInspector]
   ...
   // [HideInInspector]
    ...
    [Range(0.05f, 2.0f)]
    public float upperTargetMultiplier = 0.09f;
    public UnityEngine.UI.Slider upperTargetMultiplierSlider, channel7Slider, channel8Slider;
    ...
    // int[] targetElements = {29, 32, 35, 37, 40, 43, 44, 48};
  ...
  //  [Header("DMX Channel 14")]
   ...
   // [Header("DMX Channel 16")]
    ...
    // public void _SetUpperMultiplier()
    // {
    //     upperTargetMultiplier = upperTargetMultiplierSlider.value;
    //     upperTargetMultiplierSliderText.text = upperTargetMultiplier.ToString();
    // }
    // public int _SetChannel7()
    // {
    //     channel7SliderText.text = "Channel 7: " + channel7Slider.value.ToString();
    //     return Mathf.FloorToInt(channel7Slider.value);
    // }
    // public int _SetChannel8()
    // {
    //     channel8SliderText.text = "Channel 8: " + channel8Slider.value.ToString();
    //     return Mathf.FloorToInt(channel8Slider.value);
    // }
    ...
    void Update()
    {
        ...
        if(currentTime >= updateRate)
        {
            ...
            // {
            //     count++;
            //     float x = item * 1000000.0f;
            //     if(count >= 35) 
            //     {
            //         x *= 2.0f;
            //     } 
            //     //if(count >= 44){x *= 10000.0f;}
            //     x = Mathf.Round(x * 100f) / 100f;
            //     result += "Band " + count + ":   "; 
            //     result += x.ToString();
            //     result+="\n";
            // }
            // rawBandValueText.text = result;
            // source.GetSpectrumData(zx, 0, FFTWindow.Rectangular);
            // for(int i = 0; i < zx.Length; i++)
            // {
            //     zx[i]*= 2000000.0f;
            //     float q = (Mathf.Round(zx[i] * 100f) / 100f);
            //     string y = "Band " + i + ": " + q + "\n";
            //     rawBandValueText.text += y; 
            // }
         //   for(int i = 0; i < data.Length; i++){data[i]*= 1000000.0f; if(i >= 35) {data[i] += (i * 145.0f);} data[i] = Mathf.Round(data[i] * 100f) / 100f; data[i] = data[i] < 14050.0f || i >= 46 
...
public class BoolStyle
    ...
    [CustomEditor(typeof(VRSL_AudioDMX))]
    public class VRSL_AudioDMX_Live_Inspector : Editor
    {
        ...
        SerializedProperty m_source, m_twelveBitMode, m_updateRate, m_threshold, m_showRawData, m_data, m_dmxValueTexts, m_dmxBoolTexts, m_rawDataText, m_preCountWaitTime, m_upperTargetMultiplier, m_upperTargetMultiplierSlider, m_upperTargetMultiplierSliderText;
        SerializedProperty m_channel7Slider, m_channel8Slider, m_channel7SliderText, m_channel8SliderText;
        ...
        void OnEnable()
        {
            ...
            m_dmxValueTexts = serializedObject.FindProperty("dmxValueTexts");
            m_dmxBoolTexts = serializedObject.FindProperty("dmxBoolTexts");
            m_rawDataText = serializedObject.FindProperty("rawBandValueText");
            m_preCountWaitTime = serializedObject.FindProperty("preCountWaitTime");
            m_upperTargetMultiplier = serializedObject.FindProperty("upperTargetMultiplier");
            m_upperTargetMultiplierSlider = serializedObject.FindProperty("upperTargetMultiplierSlider");
            m_upperTargetMultiplierSliderText = serializedObject.FindProperty("upperTargetMultiplierSliderText");
            m_channel7Slider = serializedObject.FindProperty("channel7Slider");
            m_channel8Slider = serializedObject.FindProperty("channel8Slider");
            m_channel7SliderText = serializedObject.FindProperty("channel7SliderText");
            m_channel8SliderText = serializedObject.FindProperty("channel8SliderText");
    ...
    {
        VRSL.EditorScripts.VRSL_AudioDMX_Editor e = Resources.FindObjectsOfTypeAll<VRSL.EditorScripts.VRSL_AudioDMX_Editor>()[0];
        ...
        public override void OnInspectorGUI()
        {
            ...
            audioDmx = target as VRSL_AudioDMX;
            //base.OnInspectorGUI();
            ...
            if(GUILayout.Button("Copy Channel Settings From Editor Mode Version"))
            {
                #if !COMPILER_UDONSHARP && UNITY_EDITOR
                    _CopyEditorReferences();
                #endif
                audioDmx.ApplyProxyModifications();
            }
          //  EditorGUILayout.PropertyField(m_preCountWaitTime);
          //  EditorGUILayout.PropertyField(m_upperTargetMultiplier);
          //  EditorGUILayout.PropertyField(m_upperTargetMultiplierSlider);
         //   EditorGUILayout.PropertyField(m_channel7Slider);
         //   EditorGUILayout.PropertyField(m_channel7SliderText);
          //  EditorGUILayout.PropertyField(m_channel8Slider);
         ///   EditorGUILayout.PropertyField(m_channel8SliderText);
            ...
            if(audioDmx.showRawData)
            {
                ...
                EditorGUILayout.PropertyField(m_dmxBoolTexts);
                EditorGUILayout.PropertyField(m_dmxValueTexts);
                EditorGUILayout.PropertyField(m_rawDataText);
            }
